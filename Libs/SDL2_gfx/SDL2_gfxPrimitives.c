...
\returns Returns 0 on success, -1 on failure.
*/
int hline1(SDL_Renderer * renderer, Sint16 x1, Sint16 x2, Sint16 y)
{
	...
	do {
		...
		if (ocy != cy) {
			if (cy > 0) {
				ypcy = y + cy;
				ymcy = y - cy;
				result |= hline1(renderer, xmcx, xpcx + dx, ypcy + dy);
				result |= hline1(renderer, xmcx, xpcx + dx, ymcy);
			} else {
				result |= hline1(renderer, xmcx, xpcx + dx, y);
			}
			ocy = cy;
		}
		if (ocx != cx) {
			if (cx != cy) {
				if (cx > 0) {
					ypcx = y + cx;
					ymcx = y - cx;
					result |= hline1(renderer, xmcy, xpcy + dx, ymcx);
					result |= hline1(renderer, xmcy, xpcy + dx, ypcx + dy);
				} else {
					result |= hline1(renderer, xmcy, xpcy + dx, y);
				}
			}
			ocx = cx;
		}
	...
	do {
		...
		if (ocy != cy) {
			if (cy > 0) {
				ypcy = y + cy;
				ymcy = y - cy;
				result |= hline1(renderer, xmcx, xpcx, ypcy);
				result |= hline1(renderer, xmcx, xpcx, ymcy);
			} else {
				result |= hline1(renderer, xmcx, xpcx, y);
			}
			ocy = cy;
		}
		if (ocx != cx) {
			if (cx != cy) {
				if (cx > 0) {
					ypcx = y + cx;
					ymcx = y - cx;
					result |= hline1(renderer, xmcy, xpcy, ymcx);
					result |= hline1(renderer, xmcy, xpcy, ypcx);
				} else {
					result |= hline1(renderer, xmcy, xpcy, y);
				}
			}
			ocx = cx;
		}
	...
	if (rx > ry) {
		...
		do {
			...
			if ((ok != k) && (oj != k)) {
				...
				xmh = x - h;
				if (k > 0) {
					result |= hline1(renderer, xmh, xph, y + k);
					result |= hline1(renderer, xmh, xph, y - k);
				} else {
					result |= hline1(renderer, xmh, xph, y);
				}
				ok = k;
			}
			if ((oj != j) && (ok != j) && (k != j)) {
				...
				xpi = x + i;
				if (j > 0) {
					result |= hline1(renderer, xmi, xpi, y + j);
					result |= hline1(renderer, xmi, xpi, y - j);
				} else {
					result |= hline1(renderer, xmi, xpi, y);
				}
				oj = j;
			}
		...
		do {
			...
			if ((oi != i) && (oh != i)) {
				...
				xpj = x + j;
				if (i > 0) {
					result |= hline1(renderer, xmj, xpj, y + i);
					result |= hline1(renderer, xmj, xpj, y - i);
				} else {
					result |= hline1(renderer, xmj, xpj, y);
				}
				oi = i;
			}
			if ((oh != h) && (oi != h) && (i != h)) {
				...
				xpk = x + k;
				if (h > 0) {
					result |= hline1(renderer, xmk, xpk, y + h);
					result |= hline1(renderer, xmk, xpk, y - h);
				} else {
					result |= hline1(renderer, xmk, xpk, y);
				}
				oh = h;
			}
	...
	for (y = miny; (y <= maxy); y++) {
		...
		*/
		...
		for (i = 0; (i < ints); i += 2) {
			...
			result |= hline1(renderer, xa, xb, y);
		}
	}
